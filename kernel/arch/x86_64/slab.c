...
// start off with one physical page for the buckets
slab_ht ht_init() {
    slab_ht ret;
...
void kmem_cache_grow(kmem_cache* cache) {
    ...
    if (cache->size <= SMALL_OBJ_SIZE) {
        void* pstart = (void*)phys_to_virt((uintptr_t)kalloc_frame());
        ...
        *tmp = NULL;
        ...
        void* pstart = (void*)phys_to_virt((uint64_t)kalloc_frames(frames));

        // fit as many buffers as possibl
        size_t buffers = frames * PAGE_SIZE / cache->size;

        size_t slab_frames =
            (sizeof(kmem_slab) + buffers * sizeof(kmem_bufctl) + PAGE_SIZE - 1) / PAGE_SIZE;
        new_slab = (kmem_slab*)phys_to_virt((uintptr_t)kalloc_frames(slab_frames));
        new_slab->next = new_slab->prev = NULL;
        new_slab->refs = 0;
...
void _free_large_slab(kmem_cache* cache, kmem_slab *slab) {
    ...
    kfree_frames((void*)virt_to_phys(page_addr), buf_frames);
    kfree_frames((void*)virt_to_phys((uintptr_t)slab), frames);
}
...
void kmem_cache_reap(kmem_cache* cache) {
    ...
    if (cache->size <= SMALL_OBJ_SIZE) {
        while (tmp != NULL && tmp != cache->tail) {
            ...
            uintptr_t page_addr = (uintptr_t)(tmp) & ~(PAGE_SIZE - 1);
            kfree_frame((void*)virt_to_phys(page_addr));
            tmp = next;
        }
    } else {
        // kmem_slab* tmp = cache->fl_ptr;
...
void kmem_cache_free(kmem_cache* cache, void* buf) {
    ...
    if (cache->size <= SMALL_OBJ_SIZE) {
        ...
        *next = slab->freelist;
        ...
        if (!slab->refs) {
            _rm_slab(slab);
            kfree_frame((void*)virt_to_phys(page));
            alive = false;
        }
    } else {
        // TODO: Implement for large objects
        kmem_bufctl* bufctl = ht_get(&cache->buf2bufctl, (uintptr_t)buf);
        slab = bufctl->back;
        // slab->freelist
        if ((uintptr_t)bufctl->next != 0xDEADBEEF) {
            // smt is wrong
        }
